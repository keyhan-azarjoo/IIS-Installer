...
"""
    ...
    entrypoint_lines = [
        ...
        "if [ -n \"$GATEWAY_TOKEN\" ]; then",
        f'  echo "DASHBOARD URL (token,http): http://YOUR_IP:{http_port}/?token=$GATEWAY_TOKEN&gatewayUrl=ws://YOUR_IP:{http_port}"',
        f'  echo "DASHBOARD URL (token,https): {"https://YOUR_IP:" + https_port + "/?token=$GATEWAY_TOKEN&gatewayUrl=wss://YOUR_IP:" + https_port if https_port else "(disabled)"}"',
        "fi",
...
"""
    ...
    if gateway_token:
        ...
        log(f"   http://{display_host}:{http_port}/?token={gateway_token}&gatewayUrl=ws://{display_host}:{http_port}")
        if https_port:
            log(f"   https://{display_host}:{https_port}/?token={gateway_token}&gatewayUrl=wss://{display_host}:{https_port}")
        log(f"")
