...
"""
    ...
    entrypoint_lines = [
        ...
        "if [ -n \"$GATEWAY_TOKEN\" ]; then",
        f'  echo "DASHBOARD URL (token,http): http://YOUR_IP:{http_port}/app/?token=$GATEWAY_TOKEN&gatewayUrl=ws://YOUR_IP:{http_port}/gateway"',
        f'  echo "DASHBOARD URL (token,https): {"https://YOUR_IP:" + https_port + "/app/?token=$GATEWAY_TOKEN&gatewayUrl=wss://YOUR_IP:" + https_port + "/gateway" if https_port else "(disabled)"}"',
        "fi",
