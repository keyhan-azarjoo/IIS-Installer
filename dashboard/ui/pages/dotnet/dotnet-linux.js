    ...
    if (isMacOs) {
      return (
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <Alert severity="info">
              Native .NET service deployment is not automated on macOS yet. Use the Docker target for working macOS deployments from this dashboard.
            </Alert>
          </Grid>
            ...
            <Card sx={{ borderRadius: 3, border: "1px solid #dbe5f6", display: "flex", flexDirection: "column", flexGrow: 1 }}>
              <CardContent sx={{ display: "flex", flexDirection: "column", flexGrow: 1, overflow: "hidden", "&:last-child": { pb: 2 } }}>
                <Stack direction={{ xs: "column", md: "row" }} spacing={1} alignItems={{ xs: "stretch", md: "center" }}>
                  <Typography variant="h6" fontWeight={800}>Detected .NET Services</Typography>
                  <Box sx={{ flexGrow: 1 }} />
                ...
                <Box sx={{ mt: 1.2, flexGrow: 1, minHeight: "calc(100vh - 520px)", overflow: "auto" }}>
                  {linuxServices.length === 0 && (
                    <Typography variant="body2" color="text.secondary">No native .NET services detected on this macOS host.</Typography>
                  )}
                  {linuxServices.map((svc) => (
                    <Paper key={`mac-${svc.kind}-${svc.name}`} variant="outlined" sx={{ p: 1, mb: 1, borderRadius: 2 }}>
                      <Stack direction={{ xs: "column", md: "row" }} spacing={1} alignItems={{ xs: "stretch", md: "center" }}>
                        <Box sx={{ minWidth: 250 }}>
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{ borderRadius: 3, border: "1px solid #dbe5f6", height: "100%" }}>
            <CardContent>
              ...
              <Typography variant="body2"><b>HTTPS Port</b> - serve HTTPS on this port. Leave empty to disable HTTPS.</Typography>
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} sx={{ display: "flex", flexDirection: "column" }}>
