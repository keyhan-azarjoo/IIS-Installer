...
"""
    ...
    entrypoint_lines = [
        ...
        "sleep 5",
        "",
        "# Debug: show agent directory contents",
        "echo '=== Agent directory contents ==='",
        "find /root/.openclaw/agents -type f 2>/dev/null || echo 'No agents directory'",
        "echo '=== Agent config files ==='",
        "for f in /root/.openclaw/agents/main/*.json /root/.openclaw/agents/main/*.yaml /root/.openclaw/agents/main/agent/*.json; do",
        "  if [ -f \"$f\" ]; then",
        "    echo \"--- $f ---\"",
        "    cat \"$f\"",
        "    echo",
        ...
        "done",
        "",
        "# Try to update agent model using CLI",
        f"echo 'Trying to set agent model to ollama/{llm_model}...'",
        "openclaw agents list 2>&1 || echo 'agents list failed'",
        f"openclaw agents update main --model 'ollama/{llm_model}' 2>&1 || echo 'agents update failed'",
        f"openclaw agents config main --set model='ollama/{llm_model}' 2>&1 || echo 'agents config failed'",
        "",
        "# Try writing agent.json directly",
        "AGENT_JSON=/root/.openclaw/agents/main/agent.json",
        "if [ -f $AGENT_JSON ]; then",
        "  echo 'Patching existing agent.json...'",
        f"  python3 - << 'PYEOF'",
        "import json",
        "p = '/root/.openclaw/agents/main/agent.json'",
        "try: c = json.load(open(p))",
        "except: c = {}",
        f"c['model'] = 'ollama/{llm_model}'",
        "c['provider'] = 'ollama'",
        "json.dump(c, open(p, 'w'), indent=2)",
        "print('agent.json patched:', json.dumps(c, indent=2))",
        "PYEOF",
        "else",
        "  echo 'Creating agent.json...'",
        f'  echo \'{{"model":"ollama/{llm_model}","provider":"ollama"}}\' > $AGENT_JSON',
        "fi",
        "",
        "# Show final agent state",
        "echo '=== Final agent.json ==='",
        "cat /root/.openclaw/agents/main/agent.json 2>/dev/null || echo 'No agent.json'",
        "",
