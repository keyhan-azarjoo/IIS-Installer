...
"""
    ...
    entrypoint_lines = [
        ...
        "# Disable pairing — edit config YAML directly since config set may not work",
        "openclaw config set gateway.pairing.mode open 2>/dev/null || true",
        "openclaw config set gateway.dmPairing open 2>/dev/null || true",
        "openclaw config set security.pairing open 2>/dev/null || true",
        "# Also patch the YAML config file directly",
        "python3 -c \"",
        "import os, json",
        "cfg_path = os.path.expanduser('~/.openclaw/config.yaml')",
        "if os.path.exists(cfg_path):",
        "    with open(cfg_path) as f: data = f.read()",
        "    # Add pairing open mode if not present",
        "    if 'pairing' not in data:",
        "        data = data.rstrip() + '\\ngateway:\\n  pairing:\\n    mode: open\\n'",
        "    with open(cfg_path, 'w') as f: f.write(data.replace('mode: dm', 'mode: open'))",
        "    print('Config patched for open pairing')",
        "\" 2>/dev/null || true",
        "",
