...
"""
    ...
    entrypoint_lines = [
        ...
        "",
        "# Also set model in the agent's config directly",
        f"echo 'Configuring agent to use Ollama model: {llm_model}'",
        "mkdir -p /root/.openclaw/agents/main/agent",
        f'echo \'{{"model":"ollama/{llm_model}","provider":"ollama"}}\' > /root/.openclaw/agents/main/agent/model-config.json 2>/dev/null || true',
        "# Set via all possible config paths",
        f"openclaw config set defaultModel ollama/{llm_model} 2>/dev/null || true",
        f"openclaw config set agent.defaultModel ollama/{llm_model} 2>/dev/null || true",
        f"openclaw config set gateway.agent.model ollama/{llm_model} 2>/dev/null || true",
        "",
        "# Set Ollama environment variables for the gateway",
