...
"""
    ...
    entrypoint_lines = [
        ...
        "",
        "# Set Ollama env vars (will be refined later after detecting actual API URL)",
        ...
        '}',
        'AUTHEOF',
        "echo 'Auth profiles written with Ollama API URL: '$OLLAMA_API_URL",
        ...
        "rm -rf /root/.openclaw/sessions 2>/dev/null || true",
        "",
        "# Restart gateway to pick up auth-profiles.json with correct Ollama URL",
        "echo 'Restarting gateway...'",
        "kill $GW_PID 2>/dev/null || true",
        "sleep 2",
        f"OLLAMA_HOST=$OLLAMA_API_URL openclaw gateway --allow-unconfigured --bind loopback --port {gw_internal_port} --verbose &",
        "GW_PID=$!",
        "sleep 5",
        "",
        ...
        f'echo ""',
        f'echo "To use Ollama (default model is Claude):"',
        f'echo "  Open dashboard > Chat > click model dropdown > select ollama/{llm_model}"',
