...
"""
    ...
    entrypoint_lines = [
        ...
        "",
        "# If no Claude key is provided, force default model to Ollama to avoid auth failures",
        "if [ -z \"$ANTHROPIC_API_KEY\" ]; then",
        f"  echo 'No ANTHROPIC_API_KEY; defaulting agent model to ollama/{llm_model}'",
        f"  openclaw config set agents.defaults.model 'ollama/{llm_model}' 2>/dev/null || true",
        f"  openclaw config set agents.defaults.models '[\"ollama/{llm_model}\"]' 2>/dev/null || true",
        "fi",
        "",
